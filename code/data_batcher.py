...
class Batch(object):
    def __init__(self,context_mask, context_words_glove, context_words_char, question_mask, question_words_glove,question_words_char, ans_span,uuids=None):
...
class SQuadDataObject(object):
    ...
    def process_data(self,context,question,ans,discard_long):
        ...
        return glove_ids_context,glove_vectors_context,char_vectors_context,glove_ids_question,glove_vectors_question,char_vectors_question,ans_line
    ...
    def generate_one_training_epoch(self,discard_long=False):
        ...
        for i in range(num_batches):
            ...
            for (context,question,ans) in self.train_data[i*self.batch_size:(i+1)*self.batch_size]:
                glove_ids_context, glove_vectors_context, char_vectors_context, glove_ids_question, glove_vectors_question, char_vectors_question,ans_line=self.process_data(context,question,ans,discard_long)
            ...
            batch = Batch(context_mask, context_words_glove, context_words_char, question_mask, question_words_glove,question_words_char, ans_span)
            yield batch
