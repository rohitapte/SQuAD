...
from tensorflow_modules.modules import BiRNNLayer, SimpleSoftmax, BasicAttn
...
class QAModel(object):
    ...
    def build_graph(self):
        ...
        encoder = BiRNNLayer('ContextQuestionRNN',self.FLAGS.hidden_size, self.keep_prob,'LSTM')
        ...
        #add an RNN layer
        encoder2=BiRNNLayer('BlendedRepsRNN1',self.FLAGS.hidden_size,self.keep_prob,'LSTM')
        blended_hidden=encoder2.build_graph(blended_reps,self.context_mask)

        # Apply fully connected layer to each blended representation
        # Note, blended_reps_final corresponds to b' in the handout
        # Note, tf.contrib.layers.fully_connected applies a ReLU non-linarity here by default
        #blended_reps_final = tf.contrib.layers.fully_connected(blended_reps, num_outputs=self.FLAGS.hidden_size) # blended_reps_final is shape (batch_size, context_len, hidden_size)
        blended_reps_final = tf.contrib.layers.fully_connected(blended_hidden,num_outputs=self.FLAGS.hidden_size)  # blended_reps_final is shape (batch_size, context_len, hidden_size)

        # Use softmax layer to compute probability distribution for start location
        # Note this produces self.logits_start and self.probdist_start, both of which have shape (batch_size, context_len)
        with vs.variable_scope("StartDist"):
            softmax_layer_start = SimpleSoftmax()
            self.logits_start, self.probdist_start = softmax_layer_start.build_graph(blended_reps_final, self.context_mask)

        # Use softmax layer to compute probability distribution for end location
        # Note this produces self.logits_end and self.probdist_end, both of which have shape (batch_size, context_len)
        with vs.variable_scope("EndDist"):
            softmax_layer_end = SimpleSoftmax()
